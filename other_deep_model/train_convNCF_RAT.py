...
class RAT_ConvNCF(nn.Module):
    ...
    def __init__(self, user_count, item_count, enable_lat, layerlist, adv_type, adv_reg, norm,decay_factor,epsilon, pro_num,device):
        ...
        for i in range(7):
            ...
            self.register_buffer(self.y_list[y_index], torch.zeros([100, 32,32,32]))
            self.reg_size_list.append([100, 32,32,32])
            cnn_modules.append(nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size, stride=self.strides))
            cnn_modules.append(nn.ReLU())
            self.register_buffer(self.y_list[2], torch.zeros([100, 32,16,16]))
            self.reg_size_list.append([100, 32,16,16])
            cnn_modules.append(nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size, stride=self.strides))
            cnn_modules.append(nn.ReLU())
            self.register_buffer(self.y_list[3], torch.zeros([100, 32,8,8]))
            self.reg_size_list.append([100, 32,8,8])
            cnn_modules.append(nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size, stride=self.strides))
            cnn_modules.append(nn.ReLU())
            self.register_buffer(self.y_list[4], torch.zeros([100, 32,4,4]))
            self.reg_size_list.append([100, 32,4,4])
            cnn_modules.append(nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size, stride=self.strides))
            cnn_modules.append(nn.ReLU())
            self.register_buffer(self.y_list[5], torch.zeros([100, 32,2,2]))
            self.reg_size_list.append([100, 32,2,2])
            cnn_modules.append(nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size, stride=self.strides))
            cnn_modules.append(nn.ReLU())
            self.register_buffer(self.y_list[6], torch.zeros([100, 32,1,1]))
            self.reg_size_list.append([100, 32,1,1])
    ...
    def forward(self, user_ids, item_ids, is_pretrain):
        ...
        # else:
            # print(f"[DEBUG] enable_lat:{self.enable_lat}, enable_list2:{self.enable_list2}")
            if self.enable_lat:
                
                interaction_map = torch.bmm(user_embeddings.unsqueeze(2), item_embeddings.unsqueeze(1)).to(self.device)
                interaction_map = interaction_map.requires_grad_(True)  # 强制梯度保留
                self.y0 = interaction_map.view((-1, 1, self.embedding_size, self.embedding_size)).to(self.device)
                self.y0 = self.y0.requires_grad_(True)
                # print(f"[DEBUG] self.y0.requires_grad: {self.y0.requires_grad}")  # 应为 True 实为False
                self.y1 = self.cnn[0](self.y0).to(device)
                self.y1 = self.y1.requires_grad_(True)
                if self.enable_lat and 1 in self.enable_list1:
                    self.y1.requires_grad_(True)
                    self.y1.retain_grad()
                    if self.y1.grad is not None:    # 5/2 
                        self.y1.grad.zero_()  # 清空旧梯度
                ...
                self.y2 = self.cnn[2](self.z1).to(self.device) 
                self.y2 = self.y2.requires_grad_(True)
                if self.enable_lat and 2 in self.enable_list2:
                    self.y2.requires_grad_(True)
                    self.y2.retain_grad()
                ...
                self.y3 = self.cnn[4](self.z2).to(self.device)   
                self.y3 = self.y3.requires_grad_(True)
                ...
                self.y4 = self.cnn[6](self.z3).to(self.device)  
                self.y4 = self.y4.requires_grad_(True)
                if self.enable_lat and 4 in self.enable_list4:
                    self.y4.requires_grad_(True)
                    self.y4.retain_grad()
                ...
                self.y5 = self.cnn[8](self.z4).to(self.device) 
                self.y5 = self.y5.requires_grad_(True) 
                ...
                self.y6 = self.cnn[10](self.z5).to(self.device)  
                self.y6 = self.y6.requires_grad_(True)
                if self.enable_lat and 6 in self.enable_list6:
                    self.y6.requires_grad_(True)
                    self.y6.retain_grad()
        ...
        if self.enable_lat:
            if 1 in self.enable_list1:
                if self.y1.grad is not None:
                    ...
                    print(f"WARNING: y1.grad is None when processing layer 1")
    ...
    def update_seed(self):  # 5/13
        self.seed1, self.seed2, self.seed3, self.seed4, self.seed5, self.seed6 = self.random()
        self.choose_layer()  # 重新生成enable_list以更新启用的层
...
def train2(model,testRatings, testNegatives):
    args = parse_args()
    # lr = 0.05
    lr = args.lr
    epoches = 300
    ...
    t1 = time.time()
    # hr10, ndcg10 = evaluate2.metrics(model, test_loader, 10, device)  # zhaodao y0
    ...
    print(f"Init model2: HR10={np.mean(hr10):.4f}, NDCG10={np.mean(ndcg10):.4f}")
    ...
    for epoch in range(epoches):    
        ...
        #             train_data = Variable(train_data.to(self.device))
                    ...
                    #         y = getattr(model, f"y{layer}")
                    #         if layer in getattr(model, f"enable_list{layer}"):
                    #             if y.grad is None:
                    #                 print(f"[ERROR] y{layer}.grad is None after backward")
                    #             else:
                    #                 print(f"[SUCCESS] y{layer}.grad exists (mean: {y.grad.mean().item()})")
                    # loss.backward()
                    model.save_grad()   # 5/2 保存梯度
                    optimizer.step()
                    if args.enable_lat:
                        model.save_grad() 
        ...
        if hr10 > bestHr10:
            bestHr10, bestNdcg10, bestepoch = hr10, ndcgs10, epoch
            
            modelPath = f"pretrained/{args.dataset}_lr-{args.lr}_NeuMF_RAT.pth"
...
if __name__ == '__main__':
    ...
    HR10, NDCG10=train2(model2,testRatings, testNegatives)
    print('Model trained')
