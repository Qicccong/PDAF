    ...
    for layer_name, _ in model.named_parameters():
        if "weight" in layer_name and layer_name[:-len(".weight")] in layer_sharpness_dict.keys():   # 选出那些名字中包含"weight"且在layer_sharpness_dict字典中有对应项的层
            ...
            for epoch in range(1):   # 这里10改3 
                ...
                # diff = diff * scaling.view(-1, 1, 1, 1)
...
if __name__=="__main__":
    ...
    advHR10=[]
    advNDCG10=[]
    save_dir = f'pretrain/ConvNCF_PDAF/{args.data_set}_{args.lr}_{args.awp_gamma}/'
    for path in [save_dir]:
        if not os.path.isdir(path):
            os.makedirs(path)
    logger = create_logger(save_dir+'ConvNCF_PDAF_output.log')
    logger.info(args)
    args.logger = logger
    ...
    train_loader = data.DataLoader(dst, batch_size=args.batch_size, shuffle=True, num_workers=4)
    test_loader = data.DataLoader(dst, batch_size=args.test_num_ng + 1, shuffle=False, num_workers=0, drop_last=True)

    if args.data_set == "AMusic":
        num_items = 12929
    else:
        num_items = item_num
    model = ConvNCF(user_num, num_items, device=device).to(device)
    proxy_adv = ConvNCF(user_num, num_items, device).to(device)
    ...
    if args.use_pretrained:
        ...
        proxy_opt = torch.optim.Adam(proxy_adv.parameters(), lr=args.lr)
    else:
        optimizer = optim.Adagrad(model.parameters(), lr=args.lr, weight_decay=1e-2)
        proxy_opt = torch.optim.Adam(proxy_adv.parameters(), lr=args.lr)
    ...
    awp_adversary = AdvWeightPerturb1(model=model, proxy=proxy_adv, proxy_optim=proxy_opt, gamma=args.awp_gamma)
    criterion = torch.nn.BCELoss()
    best_hr1, best_ndcg1, best_epoch = 0, 0, 0
    ...
    for rounds_1 in range(5):
        sorted_layer_sharpness = layer_sharpness(args, model, train_loader, 0.008)
        ...
        hits10, ndcgs10, maps10, mrrs10 = evaluate1.evaluate_model(model, testRatings, testNegatives, K=10, K1=20, K2=50,K3=100, num_thread=1, device=device)
        HR1, NDCG1, map10,mrr10 = np.array(hits10).mean(), np.array(ndcgs10).mean(),np.array(maps10).mean(), np.array(mrrs10).mean()

        robust_HR10 = evaluate_adv.evalulate_robustness(args, model, train_loader, optimizer, args.epsilon, testRatings, testNegatives, 10, 20, 50, 100)

        logger.info(f"rounds_1:{rounds_1} init: HR10= {HR1:.4f}, NDCG10= {NDCG1:.4f}, robust HR10= {robust_HR10:.4f}")
        label_train_true = 0
        if rounds_1 == 0:
            args.init_HR10 = HR1
            best_hr1 = robust_HR10
        args.awp_adversary = AdvWeightPerturb1(model=model, proxy=proxy_adv, proxy_optim=proxy_opt, gamma=args.awp_gamma)
        for epoch in range(10):
            logger.info("==> Epoch {}".format(epoch))

            temp_awp_gamma = args.awp_gamma*(epoch+1) / 10
            awp_adversary.gamma = temp_awp_gamma
            awp_adversary.proxy.load_state_dict(model.state_dict())

            train_loss = train_model(args, model, train_loader, optimizer, loss_function)   # criterion
            ...
            logger.info(f"epoch: {epoch} init: HR10= {HR1:.4f}, NDCG10= {NDCG1:.4f}, robust HR10= {robust_HR10:.4f}")

            modelPath = save_dir + f"{args.data_set}_lr-{args.lr}_g-{args.awp_gamma}_ConvNCF_PDAF.pth"
        ...
        HR1, NDCG1, map10,mrr10 = np.array(hits10).mean(), np.array(ndcgs10).mean(),np.array(maps10).mean(), np.array(mrrs10).mean()
        robust_HR10 = evaluate_adv.evalulate_robustness(args, model, train_loader, optimizer, args.epsilon, testRatings, testNegatives, 10, 20, 50, 100)
        logger.info("==> Finetune hits10: {:.4f}%, ndcg10: {:.4f}, robust HR1={:.4f}".format(HR1, NDCG1, robust_HR10))
    
    record = interpolation(args, logger, init_sd, model_copy, model, train_loader, loss_function, save_dir)
    logger.info(record)
        
    modelPath = save_dir + f"{args.data_set}_lr-{args.lr}_g-{args.awp_gamma}_ConvNCF_PDAF_Inter.pth"
    os.makedirs("pretrained", exist_ok=True)
    torch.save(model.state_dict(), modelPath)

    print("End model1. Best epoch {:04d}: HR = {:.4f}, NDCG = {:.4f}".format(best_epoch, best_hr1, best_ndcg1))
