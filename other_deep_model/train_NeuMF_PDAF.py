...
if __name__=="__main__":
    ...
    advHR10=[]
    advNDCG10=[]
    save_dir = f'pretrain/NeuMF_PDAF/{args.data_set}_{args.lr}_{args.awp_gamma}/'
    for path in [save_dir]:
        if not os.path.isdir(path):
            os.makedirs(path)
    logger = create_logger(save_dir+'NeuMF_PDAF_output.log')
    logger.info(args)
    args.logger = logger
    ...
    for rounds_1 in range(5):
        ...
        for epoch in range(10):
            ...
            modelPath = save_dir + f"{args.data_set}_lr-{args.lr}_g-{args.awp_gamma}_NeuMF_PDAF.pth"
            if HR1 > best_hr1: 
                best_hr1 = HR1 
                best_ndcg1 = NDCG1
                best_epoch = epoch
        ...
        hits10, ndcgs10, maps10, mrrs10 = evaluate1.evaluate_model(model1, testRatings, testNegatives, K=10, K1=20, K2=50,K3=100, num_thread=1, device=device)
        HR1, NDCG1, map10,mrr10 = np.array(hits10).mean(), np.array(ndcgs10).mean(),np.array(maps10).mean(), np.array(mrrs10).mean()
        logger.info("==> Finetune hits10: {:.4f}%, ndcg10: {:.4f}".format(HR1*100, NDCG1*100))
    
    record = interpolation(args, logger, init_sd, model_copy, model1, train_loader, loss_function, save_dir)
    logger.info(record)
        
    modelPath = save_dir + f"{args.data_set}_lr-{args.lr}_g-{args.awp_gamma}_NeuMF_PDAF_Inter.pth"
