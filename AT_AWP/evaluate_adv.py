...
def eval_one_rating(idx, device):
    ...
    with torch.no_grad():
        ...
        # loss,_,_,_,_,_,_,ly= model_stabel(users, items,lbl)

        # 分布式多GPU训练
        users = torch.from_numpy(users)
        items1 = torch.tensor(items)
        # lbl = torch.from_numpy(lbl).to(torch.float32)
        loss,_,_,_,_,_,_,ly= model_stabel(users.to(device_ids[0]), items1.to(device_ids[0]),lbl.to(device_ids[0]))
        # loss2,_,_,_,_,_,_,ly= model_stabel(users, items,lbl)
    ...
    if attack == 'fgsm': 
        ...
        for ui, ii, lbl in ldr:
            ...
            # 分布式多GPU训练
            loss,_,_,_,_,_,_,ly= _model(ui.to(device_ids[0]), ii.to(device_ids[0]),lbl.to(device_ids[0]))
            # loss = loss1[0]+ loss1[1]
                    ...
                    for name, param in _model.named_parameters():
                        ...
                        if "weight" in name:
                            if random_start and param.requires_grad:
                                # 初始化随机扰动
                                size_para = param.size(0)
                                random_noise = epsilon * torch.rand_like(param)
                                size_para = param.size(0)
                                r_at = random_noise
                                normVal1 = torch.norm(r_at.view(size_para, -1), 2, 1)
                                normVal2 = torch.norm(param.view(size_para, -1), 2, 1)
    ...
    with torch.no_grad():
        for ui, ii, lbl in ldr:
            ui, ii, lbl = ui.to(device_ids[0]), ii.to(device_ids[0]), lbl.to(device_ids[0])
            bsz = ui.size(0)
            if torch.any(lbl > 1):
                lbl[(lbl > 1) | (lbl < 0)] = 0
