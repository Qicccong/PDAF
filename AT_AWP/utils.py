...
def interpolation(args, logger, init_sd, ft_sd, model, trainloader, criterion,save_dir):
    ...
    for alpha in alphas:
        ...
        for name, _ in init_sd.items():
            ...
            model_dict[name] = model_dict[name].to(device)  #### 1109
        ...
        test_robust_hr = evalulate_robustness(args, model,trainloader,args.optimizer,args.epsilon,
                                                   args.testRatings, args.testNegatives, args.topK,args.topK1,args.topK2,args.topK3)

        logger.info("==> Alpha: {:.4f}, hits10: {:.4f}%, test robust hit10: {:.4f}%".format(alpha, hr10*100, test_robust_hr*100))
        records.append((hr10, test_robust_hr))

        # 使用hr10和test_robust_acc的综合值来评估性能，这里简单地取它们的平均值作为示例
        # current_performance = (hr10 + test_robust_hr) / 2
        current_performance = hr10

        if alpha == 0:
            test_robust_acc_init = test_robust_hr
            best_performance = current_performance
        # 更新最优模型


        if test_robust_hr >=test_robust_acc_init:
            if current_performance >= args.init_HR10*0.985:
                best_alpha = alpha
                test_robust_acc_init = test_robust_hr
    ...
    if best_checkpoint_path:
        best_model_dict = torch.load(best_checkpoint_path, map_location=torch.device('cpu'), weights_only=True)
