...
import numpy as np
from mlp import MLP,MLP_new1,MLP_white_attack_test
import torch
import torch.nn as nn
from Dataset import Dataset

from AT_AWP.evaluate import ( evaluate_model, get_train_matrix)
...
def parse_args():
    ...
    parser.add_argument("--epsilon", nargs="?", default=0.5) # 0.6
    parser.add_argument("--alpha", nargs="?", default=0.5/25)
    parser.add_argument("--pro_num", nargs="?", default=25, choices=[1,25])
    ...
    parser.add_argument("--adv_type1", nargs="?", default="mim ", choices=['fgsm', 'bim', 'pgd',"mim"])
...
class advMLP(nn.Module):
    ...
    def forward(self, user, item,label):
        ...
        if self.adv_type=="fgsm":
            ...
            _, self.l1,self.l3,self.l5,self.l7,self.l9,self.l11, ly = self.model(user,item, label)
            ly = ly.squeeze()
        ...
        if self.adv_type=="bim":
            ...
            for i in range(self.pro_num):
                ...
                if i==0:
                    ...
                    self.l1,self.l3,self.l5,self.l7,self.l9,self.l11, ly = self.new_model(y, user,item, label)
                ly = ly.squeeze()
                loss = criterion(ly, label)
                self.l1.register_hook(save_grad('1'))
                loss.register_hook(save_grad('loss'))
...
if __name__ == "__main__":
    ...
    modelPath = "/mnt/home/E22301274/Def/AWP-main/clean_lastfm_1e-05_mim_MLP_robust.pth"
    print(modelPath)
    ...
    model = MLP_white_attack_test(fcLayers, userMatrix, itemMatrix, alpha=args.alpha, epsilon=args.epsilon, pro_num=args.pro_num,\
                enable_lat=args.enable_lat, layerlist=args.layerlist, adv_type=args.adv_type1,adv_reg=args.adv_reg, norm=args.norm,decay_factor=args.decay_factor, device=device)
    ...
    new_model = MLP_new1(model,userMatrix, itemMatrix, device)
    new_model.to(device)
